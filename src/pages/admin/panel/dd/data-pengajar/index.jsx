...
function Index() {
  ...
  const submitHandlerUpdate = async (newData, oldData) => {
    ...
    try {
      await db.collection('pengajar').doc(newData.id).update(newData);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const tryUpdate = async () => {
    ...
    try {
      ...
      if (res == 1) {
        const urlh = await uploadImagenGetURL(file, "pengajar");
        await db.collection("pengajar").doc(data.id).update({
          ...data,
          url:urlh
        });
      }
          ...
          <Col breakPoint={{ xs: 12 }}>
            <Card accent="Primary">
              <CardHeader>
                <Row center around>
                  Daftar Pengajar
                  <Button status="Primary" onClick={() => router.push(`${router.pathname}/addPartners`)}>
              ...
              <MaterialTable
                ...
                columns={[
                  { title: 'Nama', field: 'title' },
                  { title: 'Bidang', field: 'desc' },
                  {
                    title: 'Foto',
                    render: (rowData) => (
                      <ImageWrapper>
